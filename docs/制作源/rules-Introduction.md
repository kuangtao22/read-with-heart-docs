...
| `openParams.<key>.type` | `@{openParams.line.type}` | 开放参数类型 | 仅用于本地配置页结构渲染。 |
...
| 宿主注入别名后 `setOpenParams(k, v)` | 条件可用 | 给规则作者提供更短、更自然的 JS API | `setOpenParams(k, v)` | 当前未内置该别名；需要宿主先注入 `window.setOpenParams = function(key, value) { window.ParsingBook.setOpenParamValue(key, value); }`。 |
