...
`ruleSearch.aliasName` 是可选的书籍别名解析规则，适合解析站点返回的又名、原名、译名等额外书名。别名规则为空或解析结果为空时不会参与匹配；有值时，搜索结果筛选和精准/包含分组会按 `bookName OR aliasName` 判断，任意一个命中关键词即可匹配。非空别名会作为搜索结果和加入书架后的显示书名使用。
